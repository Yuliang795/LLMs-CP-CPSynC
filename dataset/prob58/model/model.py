...
def ref_model(param_dict):
  nClientsPerWarehouse= param_dict['PSize']
...
if __name__ == '__main__':
    ...
    print(dvar_dict)
    #
    depot, xa, xb, da, db = ref_model(param_dict)
    #
    print(xa)
    #
    ...
    if args.check == 'sat':
        satisfy(
            depot == dvar_dict["depot"],
            xa == dvar_dict["xa"],
            xb == dvar_dict["xb"],
            Maximum(Sum(da), Sum(db)) == dvar_dict["z"],
        )
        # display the result
    ...
    # verify optimality of the solution
        ...
        if solve() in [SAT, OPTIMUM]:
            print(f"opt@SUBOPT |  |ref:{max(sum(values(da)), sum(values(db)))} - sol:{dvar_dict['z']}")
        else:
            print("opt@OPT")